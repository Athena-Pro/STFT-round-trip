...
export class SpectralMask {
  ...
  idx(f: number, b: number) { return f * this.B + b; }
  ...
  applyBrush(centerF: number, centerB: number, radiusB: number, gainDb: number, isErase: boolean, brushMode: 'subtractive' | 'generative') {
    const layer = brushMode === 'subtractive' ? this.gainDbLayer : this.generativeDbLayer;
    const neutralValue = brushMode === 'subtractive' ? 0 : -999;
    const radiusF = Math.max(2, Math.round(radiusB / 4));

    const f0 = Math.max(0, Math.floor(centerF - radiusF)), f1 = Math.min(this.F - 1, Math.ceil(centerF + radiusF));
    const b0 = Math.max(0, Math.floor(centerB - radiusB)), b1 = Math.min(this.B - 1, Math.ceil(centerB + radiusB));
      ...
      for (let b = b0; b <= b1; b++) {
        ...
        if (distSq <= 1) {
          ...
          if (isErase) {
            ...
            else { // Feather erase for subtractive
...
const applyAudioGlitches = (signal: Float32Array, params: StftParams, transformParams: TransformParams): Float32Array => {
    ...
    for (let i = 0; i < output.length; i += chunkSize) {
        if (Math.random() < glitch.stutterChance) {
             const stutterDurationSamples = Math.floor(sr * (glitch.stutterDuration / 1000));
             const stutterChunk = output.slice(Math.max(0, i-chunkSize), i);
             if (stutterChunk.length > 0) {
                 for(let s=0; s < stutterDurationSamples; s++) {
                     if (i+s < output.length) output[i+s] = stutterChunk[s % stutterChunk.length];
                 }
                 i += stutterDurationSamples - chunkSize;
             }
        }
        if (Math.random() < glitch.dropChance) {
            for(let j=0; j<chunkSize && i+j < output.length; j++) output[i+j] = 0;
        }
    }
    return output;
};
...
const istftCooperative = async (ZxxTransposed: number[][][], params: StftParams, originalLength: number): Promise<Float32Array> => {
    const { nfft, hopLength } = params;
    ...
    for (let frameIndex = 0; frameIndex < Zxx.length; frameIndex++) {
        ...
        for (let i = frame.length - 2; i > 0; i--) fullFrame.push([frame[i][0], -frame[i][1]]);

        const complexFrameInput = fft.createComplexArray();
        let k = 0;
        for (const [re, im] of fullFrame) { complexFrameInput[k++] = re; complexFrameInput[k++] = im; }
...
const generateSpectrogramData = (stftResult: number[][][], params: StftParams): SpectrogramData => {
    if (!stftResult || stftResult.length === 0) return { data: [], freqLabels: [] };
    
    const dbMatrix = stftResult.map(row => row.map(c => 20 * Math.log10(Math.sqrt(c[0]**2 + c[1]**2) + 1e-12)));
    
    const DB_FLOOR = -90;
    const DB_CEIL = 0;
    const DB_SPAN = DB_CEIL - DB_FLOOR;
    const normalizedMatrix = dbMatrix.map(row => row.map(db => {
    ...
    const downsampleAndCreateLabels = (matrix: number[][]): SpectrogramData => {
        const freqBins = matrix.length, timeSteps = matrix.length > 0 ? matrix[0].length : 0;
        const MAX_DISPLAY_TIME_STEPS = 512, MAX_DISPLAY_FREQ_BINS = 256;
        const timeRatio = Math.max(1, Math.floor(timeSteps / MAX_DISPLAY_TIME_STEPS));
...
export const resynthesizeAudio = async (
    ...
    let reconstructedSignal = await istftCooperative(maskedStft, params, originalLength);

    // Apply any time-domain transforms like audio glitches.
    reconstructedSignal = applyAudioGlitches(reconstructedSignal, params, transformParams);
