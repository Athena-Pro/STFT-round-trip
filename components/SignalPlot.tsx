...
const SignalPlot: React.FC<SignalPlotProps> = ({ originalData, reconstructedData }) => {
    
    // Combine data for plotting on the same chart
    const combinedData = originalData.map((point, index) => ({
        time: point.time,
        original: point.amplitude,
        reconstructed: reconstructedData[index] ? reconstructedData[index].amplitude : null,
    }));
        ...
        <div className="w-full h-80 bg-slate-50 p-2 rounded-lg border">
            <ResponsiveContainer>
                ...
                >
                    ...
                    <Line type="monotone" dataKey="original" name="Original Signal" stroke="#3b82f6" dot={false} strokeWidth={2} />
                    <Line type="monotone" dataKey="reconstructed" name="Reconstructed Signal" stroke="#ef4444" strokeDasharray="3 3" dot={false} strokeWidth={2}/>
                </LineChart>
            </ResponsiveContainer>
        </div>
    );
