...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  if (isLossless) {
    if (isSuccess) {
      ...
      message =
        'The chosen parameters satisfy COLA conditions, demonstrating a high-fidelity reconstruction.';
      icon = (
        <svg
      ...
      message =
        'COLA conditions are met, but error is high. This might be due to numerical precision limits.';
      icon = (
        <svg
          xmlns="http://www.w3.org/2000/svg"
    ...
    <div className="flex flex-col md:flex-row gap-4 items-start bg-slate-50 p-4 rounded-lg border">
      ...
      <div className="flex-1">
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
          <h3
            className={`text-lg font-semibold ${isSuccess ? 'text-green-700' : isLossless ? 'text-yellow-700' : 'text-blue-700'}`}
          >
            {title}
          </h3>
          ...
          {snr !== null && (
            <p className="text-sm text-slate-600 font-mono">
              SNR:{' '}
              <span
                className={`font-bold ${snr > 60 && isLossless ? 'text-green-800' : 'text-slate-800'}`}
              >
                {isFinite(snr) ? snr.toFixed(2) + ' dB' : 'Perfect'}
              </span>
            </p>
          )}
          {isResynthesizing && (
